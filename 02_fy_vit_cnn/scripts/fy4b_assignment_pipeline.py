...
FY_ROOT = Path(__file__).resolve().parents[1]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from src.evaluation import plot_grouped_history_curves, save_classification_artifacts
...
class AssignmentConfig:
    data_dir: Path = FY_ROOT / "data_processed/fy4b_vit_month_stratified_scene_split"
    runs_dir: Path = FY_ROOT / "runs/assignment_suite"
    attention_dir: Path = FY_ROOT / "attention_maps"
    tradeoff_png: Path = FY_ROOT / "speed_accuracy_tradeoff.png"
    results_csv: Path = FY_ROOT / "assignment_results.csv"
    results_json: Path = FY_ROOT / "assignment_results.json"
...
def forward_with_features(model: nn.Module, images: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    if isinstance(model, FY4BViT):
        tokens = model.forward_features(images)
        features = tokens[:, 0]
        return model.head(features), features
    if all(hasattr(model, name) for name in ["conv1", "bn1", "relu", "maxpool", "layer1", "layer2", "layer3", "layer4", "avgpool", "fc"]):
...
def evaluate_with_outputs(model: nn.Module, loader: DataLoader, criterion: nn.Module, device: torch.device, *, collect_features: bool = True) -> dict[str, Any]:
    model.eval()
    losses: list[float] = []
    all_preds: list[np.ndarray] = []
    all_targets: list[np.ndarray] = []
...
def enrich_existing_experiment(
    model_name: str,
    fraction: float,
    bundle: DataBundle,
    ...
    criterion_eval = nn.CrossEntropyLoss(weight=torch.tensor(fraction_class_weights, dtype=torch.float32, device=device))

    val_metrics = evaluate_with_outputs(model, val_loader, criterion_eval, device, collect_features=cfg.generate_tsne)
    test_metrics = evaluate_with_outputs(model, test_loader, criterion_eval, device, collect_features=cfg.generate_tsne)
    ...
    val_metric_pack = save_classification_artifacts(
        run_dir,
        "val",
        val_metrics["targets"],
        val_metrics["preds"],
        ...
        title_prefix=f"FY-4B {model_name.upper()} {fraction:.0%}",
        tsne_max_samples=cfg.tsne_max_samples,
        random_state=cfg.seed,
    )
    test_metric_pack = save_classification_artifacts(
        run_dir,
        "test",
    ...
    summary.update(
        {
            ...
            "val_acc": float(val_metric_pack["accuracy"]),
            "val_macro_f1": float(val_metric_pack["macro_f1"]),
            "val_weighted_f1": float(val_metric_pack["weighted_f1"]),
            "val_balanced_accuracy": float(val_metric_pack["balanced_accuracy"]),
            "val_macro_precision": float(val_metric_pack["macro_precision"]),
            "val_macro_recall": float(val_metric_pack["macro_recall"]),
            "test_loss": float(test_metrics["loss"]),
            "test_acc": float(test_metric_pack["accuracy"]),
            "test_macro_f1": float(test_metric_pack["macro_f1"]),
            "test_weighted_f1": float(test_metric_pack["weighted_f1"]),
            "test_balanced_accuracy": float(test_metric_pack["balanced_accuracy"]),
            "test_macro_precision": float(test_metric_pack["macro_precision"]),
            "test_macro_recall": float(test_metric_pack["macro_recall"]),
            "test_top3_accuracy": float(test_metric_pack.get("top3_accuracy", np.nan)),
            "test_cohen_kappa": float(test_metric_pack["cohen_kappa"]),
            "test_matthews_corrcoef": float(test_metric_pack["matthews_corrcoef"]),
            "val_classification_report": val_metric_pack["classification_report"],
            "test_classification_report": test_metric_pack["classification_report"],
        }
    )
...
def train_single_experiment(model_name: str, fraction: float, bundle: DataBundle, split_bundle: SplitBundle, cfg: AssignmentConfig, *, skip_existing: bool = False) -> tuple[dict[str, Any], Path]:
    ...
    if skip_existing and summary_path.exists() and checkpoint_path.exists():
        ...
        required = [run_dir / "test_metrics.json", run_dir / "test_confusion_matrix_normalized.png", run_dir / "test_predictions.csv"]
        if cfg.generate_tsne:
            required.append(run_dir / "test_tsne.png")
        if all(path.exists() for path in required):
            return summary, checkpoint_path
        print(f"[{model_name} {fraction:.0%}] Existing checkpoint found; enriching unified evaluation artifacts.")
        return enrich_existing_experiment(model_name, fraction, bundle, split_bundle, cfg, run_dir, checkpoint_path, summary)
    ...
    summary = {
        ...
        "val_loss": float(val_metrics["loss"]), "val_acc": float(val_metric_pack["accuracy"]), "val_macro_f1": float(val_metric_pack["macro_f1"]), "val_weighted_f1": float(val_metric_pack["weighted_f1"]),
        "val_balanced_accuracy": float(val_metric_pack["balanced_accuracy"]), "val_macro_precision": float(val_metric_pack["macro_precision"]), "val_macro_recall": float(val_metric_pack["macro_recall"]),
        "test_loss": float(test_metrics["loss"]), "test_acc": float(test_metric_pack["accuracy"]), "test_macro_f1": float(test_metric_pack["macro_f1"]), "test_weighted_f1": float(test_metric_pack["weighted_f1"]),
        "test_balanced_accuracy": float(test_metric_pack["balanced_accuracy"]), "test_macro_precision": float(test_metric_pack["macro_precision"]), "test_macro_recall": float(test_metric_pack["macro_recall"]),
        "test_top3_accuracy": float(test_metric_pack.get("top3_accuracy", np.nan)),
        "test_cohen_kappa": float(test_metric_pack["cohen_kappa"]), "test_matthews_corrcoef": float(test_metric_pack["matthews_corrcoef"]),
...
def run_full_suite(args: argparse.Namespace) -> None:
    ...
    cfg.generate_tsne = not args.no_tsne
    cfg.tsne_max_samples = args.tsne_max_samples if args.tsne_max_samples is not None else cfg.tsne_max_samples
    ...
    for model_name in args.models:
        run_dirs = sorted(cfg.runs_dir.glob(f"{model_name}_fraction_*"))
        plot_grouped_history_curves(run_dirs, cfg.runs_dir, model_name=model_name, dataset_name="FY-4B")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    full.add_argument("--no-tsne", action="store_true", help="Skip t-SNE feature visualization.")
    full.add_argument("--tsne-max-samples", type=int, default=None)
